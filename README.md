...
| **업무일지** | 읽기 화면 우선 · 묶음 업무의 모든 물건 표시 · 기존 매물 주소·가격 불러오기 · 작성 중 고객·매물 이력 조회 |
| **고객 관리** | 이름·전화번호로 찾고, 전체 상담 이력을 확인한 뒤 다음 연락할 일 등록 |
| **매물 관리** | 건물·동·호수별 조회 · 기존 이력을 보존하는 매물 수정 기록 · 현재 정보와 이력의 원자적 저장 · 미갱신 매물 점검 |
...
| 검증·기능 | 값 | 의미 |
| :--- | ---: | :--- |
| 자동 테스트 | **277개 통과** | 실제 API/SQL와 화면 이벤트로 엑셀 달력 규칙·읽기 우선·전체 물건·원자적 이력 저장·검색·복귀·백업 등 회귀 검증 |
