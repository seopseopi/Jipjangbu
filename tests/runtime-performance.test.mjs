...
const tables = ["customers", "work_logs", "work_log_properties", "listings", "listing_events", "work_types", "property_buildings", "follow_ups"];
...
test("초기화 완료된 DB는 새 서버에서도 읽기 1회, 쓰기 0회로 준비되고 동시 요청을 공유한다", async (t) => {
  ...
  assert.equal(sqlite.prepare("SELECT value FROM app_runtime_state WHERE key = 'bootstrap_version'").get().value, "1");
...
test("전체 백업은 8개 테이블을 한 batch로 읽고 데이터·암호화 형식을 그대로 보존한다", async (t) => {
