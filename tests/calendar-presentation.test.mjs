...
test("엑셀의 매물 관련 5종은 일반 고객일 때 물건 주소만 우선 표시한다", () => {
  for (const work_type of ["매물등록", "매물수정", "매물취소", "타계약확인", "경매확인"]) {
    ...
    assert.deepEqual(result.entries, [address], work_type);
  }
});
...
test("엑셀의 계약·금전 5종은 일반 고객일 때 각 물건의 물건지·업소를 표시한다", () => {
  for (const work_type of ["가계약", "계약서작성", "잔금", "중도금", "계약취소"]) {
    ...
    for (const source of ["", "  ", "마전현대", " 마전현대 "]) {
      assert.deepEqual(calendarWorkPresentation(work({ work_type, properties_json: JSON.stringify([property({ source })]) })).entries, [`${address} (단독)`]);
    }
  }
});

test("부동산 고객과 자체결정의 물건 업무는 물건지 대신 고객ID로 구분한다", () => {
  for (const customer_name of ["검증부동산", "부동산", "자체결정"]) {
    for (const work_type of ["매물등록", "매물수정", "매물취소", "가계약", "계약서작성", "잔금", "중도금", "계약취소", "타계약확인", "경매확인"]) {
      assert.deepEqual(calendarWorkPresentation(work({ customer_name, work_type })).entries, [`${address} (고객ID-검증)`]);
    }
  }
});

test("부동산써브는 원본과 동일하게 부동산 고객ID 표시 예외로 처리한다", () => {
  assert.deepEqual(calendarWorkPresentation(work({ customer_name: "부동산써브" })).entries, [address]);
...
test("전화·방문·예정·계약파기·신규 업무는 연결 물건이 있어도 고객을 먼저 표시한다", () => {
  for (const work_type of ["전화", "집방문", "잔금예정", "계약예정", "계약파기", "새 업무종별"]) {
    ...
    assert.deepEqual(result.properties.map((item) => item.label), [address]);
...
test("다중 물건은 모두 원래 순서로 표시하고 계약업무의 물건지·업소도 각각 적용한다", () => {
  ...
  assert.deepEqual(result.entries, [`${address} (단독)`, "두번째단지 106동 1503호 (다른업소)", "세번째단지 106동 1503호 (단독)"]);
