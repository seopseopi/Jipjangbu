...
export async function GET() {
  try {
    await ready();
    const db = getD1();
    const [summaryRow, monthlyRows, workTypeRows, upcomingRows, staleListingRows] = await Promise.all([
      db.prepare(`
        SELECT
          (SELECT COUNT(*) FROM work_logs
            WHERE substr(work_date, 1, 7) = strftime('%Y-%m', 'now', '+9 hours')) AS month_work_count,
          (SELECT COUNT(DISTINCT customer_id) FROM work_logs
            WHERE substr(work_date, 1, 7) = strftime('%Y-%m', 'now', '+9 hours')) AS month_customer_count,
      ...
      db.prepare(`
        ...
        FROM month_range
        LEFT JOIN work_logs ON substr(work_logs.work_date, 1, 7) = month_range.month
      ...
      db.prepare(`
        ...
        FROM work_logs
        WHERE substr(work_date, 1, 7) = strftime('%Y-%m', 'now', '+9 hours')
        GROUP BY work_type
    ...
    return Response.json({
      ...
      monthly: monthlyRows.results.map((row) => ({ month: row.month, count: Number(row.count) })),
      workTypes: workTypeRows.results.map((row) => ({ name: row.name, count: Number(row.count) })),
