...
export function getPropertyDisplayGroups(properties: WorkPropertySummary[]): Array<{
  key: string;
  building: string;
  items: Array<{ property: WorkPropertySummary; index: number; shortLabel: string; sourceLabel: string }>;
}> {
  const groups: Array<{
    key: string;
    building: string;
  ...
  properties.forEach((property, index) => {
    ...
    if (building && previous?.building === building && previous.items[0].property.property_type.trim() === type) {
      previous.items.push({ property, index, shortLabel, sourceLabel });
    } else {
      groups.push({ key: `${index}|${type}|${building}`, building, items: [{ property, index, shortLabel, sourceLabel }] });
    }
  });
  return groups;
}
