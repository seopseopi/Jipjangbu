...
export async function rebuildListings(keys: Iterable<string>) {
  ...
  for (const key of new Set([...keys].filter(Boolean))) {
    ...
    const history = await db.prepare(`
      SELECT event_date, status, notes FROM listing_events
      WHERE listing_key = ? ORDER BY event_date DESC, created_at DESC
    `).bind(key).all<{ event_date: string; status: string; notes: string }>();
    ...
    `).bind(
      ...
      latest.sale_price, latest.jeonse_price, latest.monthly_rent, notes, latest.is_demo,
    ).run();
  }
}
